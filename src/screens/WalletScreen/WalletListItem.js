...
const WalletListItem = ({item}) => {
  ...
  return (
    <View>
      <View style={styles.container} className="flex-row py-3 gap-2">
          ...
          <TouchableWithoutFeedback onPress={() => toggleListItem()}>
            <View className="flex ">
              <View className="flex gap-1 ">
                <View className=" flex-row justify-between items-center">
                  ...
                  <View className="flex flex-row gap-3 items-center justify-start">
                    <MaterialIcons
                      name="circle"
                      size={10}
                      color={item?.verified ? '#05BC61' : '#FFA500'}
                    />
                    <Animated.View
                ...
                {!item?.verified ? (
                  <Text className="text-xs text-[#FFA500]">
                    Requires Update
                  </Text>
                ) : (
            ...
            <View style={styles.body}>
              <View className="bg-[#F8FBF9] p-3 mb-5 space-y-3">
                ...
                <View className={`${item.verified && 'hidden'}`}>
                  <Text className="text-[#FFA500] font-bold">
                    Update Requires:
                  </Text>
                  <Text className="text-gray-500">
                    {item.body.updateRequire}
                  </Text>
                </View>
                ...
                <View>
                  ...
                  <Text className="text-gray-500">
                    ...
                    <Text className="font-bold"> {item?.body?.app} App</Text>
                  </Text>
                </View>
              </View>
